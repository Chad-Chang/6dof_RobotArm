...
commu = Communication('127.0.1.1',2013,'/dev/ttyUSB1');
...
shelve1_finish = 26;shelve2_finish = 27; 
...
while(working)
    ...
    %% orig_P
        case orig_P
            ...
            disp("th1 turn")
            if(~is_shelve1) % if 1st shelve is empty
                disp("first shelve is empty");
                robot_op.m_status = shelve1;
            else % if 1st shelve is filled
                disp("first shelve is filled");
                if(~is_shelve2) % if 2nd shelve is empty
                    disp("second shelve is empty");
                    robot_op.m_status = shelve2;
                else % if 2nd shelve is filled
                    disp("second shelve is filled");
    ...
    %% shelve1
        case shelve1
            ...
            disp("bookshelf1")
            is_shelve1 = true;
            robot_op.m_status = shelve1_finish;

    %% shelve1_finished || shelve2_finished
        case shelve1_finish || shelve2_finish
            disp("status = shelve1 or shelve2");
            robot_op.m_status
            commu.send_s(s,shelve1_f,[0,0,0,0,0,0]);
            data_uart = commu.receive_s(s);
            if(data_uart == "s")
                robot_op.m_status = m_return;
            end
    ...
    %% shelve2
        case shelve2
            ...
            disp("bookshelf2");
            is_shelve2 = true;
            robot_op.m_status = shelve2_finish;
%     %% pick&place1_1
%         case pick_place1_1
%             
%     %% pick&place1_2
